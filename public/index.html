...
import { 
  Activity, Mountain, AlertTriangle, Target, Battery, Wifi, Signal,
  ShieldAlert, CheckCircle2, Navigation, History, Trash2, Plus,
  Brain, Dumbbell, Coffee
} from 'lucide-react';
...
const SliderControl = ({ label, value, onChange, colorClass = "accent-emerald-500" }) => (
  <div className="mb-3">
...
const PageRadar = ({ data, updateData }) => {
  ...
  const chartData = [
    ...
    { subject: 'Medias', A: data.media, fullMark: 100 },
  ...
  const analyzeSignal = () => {
    if (data.fog <= 30) {
        return { 
            ...
            msg: "Visibilite excellente. Conditions de vol ideales." 
        };
    }
    else if (data.fog <= 70) {
        return { 
            ...
            msg: "Visibilite reduite. Soyez vigilant aux interferences." 
        };
    }
    else {
        return { 
            ...
            msg: "Visibilite nulle. Arret immediat conseille." 
        };
    }
  };
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Header title="Systeme de Navigation" subtitle="Calibrage des Capteurs" />
      
      <Card className="relative overflow-hidden group">
        <div className="flex items-center justify-between mb-2">
          <span className="text-slate-400 text-sm font-mono">DENSITE BROUILLARD</span>
      ...
      <Card>
        ...
        <div className="space-y-2 bg-black/20 p-3 rounded-xl border border-white/5">
          <SliderControl label="Voix Interieure (Moi)" value={data.inner} onChange={(v) => updateData('inner', v)} colorClass="accent-sky-500" />
          <div className="grid grid-cols-2 gap-x-4">
            ...
            <SliderControl label="Medias" value={data.media} onChange={(v) => updateData('media', v)} colorClass="accent-red-400" />
      ...
      <div className={`bg-slate-800 border-l-4 rounded-r-xl p-4 flex gap-3 items-start ${analysis.borderColor}`}>
        ...
        <div>
          <h3 className={`font-bold text-sm mb-1 ${analysis.color}`}>ANALYSE SYSTEME</h3>
...
const PageAscension = ({ data, updateData }) => {
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Header title="Deploiement Strategique" subtitle="Altitude & Materiel" />
      ...
      <div className="space-y-3">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <input 
                ...
                placeholder="Ex: 20 Pompes au reveil"
                className="w-full bg-transparent text-sm text-white placeholder:text-slate-600 focus:outline-none"
            />
          </div>
        </div>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <label className="text-[10px] text-blue-400 font-bold">RECUPERATION (Off)</label>
            <input 
                type="text" 
...
const PageCockpit = ({ logs, addLog, clearLogs, crisisData, updateCrisis }) => {
  ...
  return (
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Header title="Protocole de Resilience" subtitle="Journal & Urgences" />
      ...
      {!crisisMode ? (
        ...
        <Card className="border-red-500/50 bg-red-950/30 animate-in zoom-in duration-300 relative">
            ...
            <h3 className="font-bold text-red-400 flex items-center gap-2 mb-4">
                <AlertTriangle size={18}/> MODE SURVIE ACTIVE
            </h3>
            
            <div className="space-y-4">
                <div className="bg-black/40 p-3 rounded border-l-2 border-red-500">
                    <p className="text-xs text-slate-400 mb-1">Ne reflechis pas. Execute :</p>
                    <p className="text-white font-mono font-bold">N'oublie pas ton objectif, ouvre ton sac et accroche toi a tes mousquetons.</p>
                </div>

                <div className="space-y-2">
                    <p className="text-xs font-bold text-slate-500 uppercase">Ancrages de Securite</p>
                    <InputField 
                    ...
                    <InputField 
                        ...
                        placeholder="Ex: Chanter Celine Dion..."
                    />
                </div>
            </div>
        </Card>
      ...
      <Card>
        <div className="flex items-center justify-between mb-3">
             ...
             <div className="flex items-center gap-2">
                <span className="text-xs bg-sky-900 text-sky-200 px-2 py-0.5 rounded-full">{logs.length} entrees</span>
                {logs.length > 0 && (
        ...
        <div className="space-y-2 max-h-40 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-700">
            {logs.length === 0 && <p className="text-xs text-slate-500 italic text-center py-2">Aucune progression enregistree.</p>}
...
const PageMission = ({ radar, goal, logCount }) => {
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Header title="Le Manifeste de Gravite" subtitle="Autorisation de Vol" />
      ...
      <Card className="aspect-square relative p-6 bg-slate-800 overflow-hidden">
        ...
        <div className="absolute left-2 top-1/2 -translate-y-1/2 -rotate-90 text-[10px] text-slate-400 tracking-widest uppercase font-bold">Niveau Maitrise</div>
        ...
        <div className="w-full h-full border border-slate-600/50 bg-[linear-gradient(rgba(51,65,85,0.1)_1px,transparent_1px),linear-gradient(90deg,rgba(51,65,85,0.1)_1px,transparent_1px)] bg-[size:20px_20px] relative">
            <div className="absolute top-0 right-0 w-1/3 h-1/3 bg-emerald-500/10 border-l border-b border-emerald-500/30 flex items-center justify-center">
                <span className="text-[10px] text-emerald-500/50 font-mono absolute top-2 right-2">GRAVITE</span>
            </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-slate-800 p-4 rounded-xl border-t-4 border-purple-500">
          <div className="text-xs text-slate-400 mb-1">MAITRISE (X)</div>
      ...
      {x > 7 && y > 7 && (
        <div className="bg-emerald-900/30 border border-emerald-500/50 p-3 rounded-xl flex items-center gap-3 animate-pulse">
            ...
            <div className="text-sm text-emerald-100 font-bold">Autorisation de Vol Validee</div>
        </div>
      )}
    </div>
  );
...
const App = () => {
  ...
  const [plusOneLogs, setPlusOneLogs] = useState([]);
  ...
  const clearLogs = () => {
    if(window.confirm("Reinitialiser l'historique de vol ?")) {
        setPlusOneLogs([]);
    }
  };

  const updateCrisis = (key, val) => {
